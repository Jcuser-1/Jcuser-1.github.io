...
**设计**：Apple 风格（#f5f5f7 浅灰底 / 白卡大圆角 / 苹果蓝）。首页 = 全屏风景视频封面（深蓝面纱 + 入场动画）；登录页 = **科技风粒子网络背景**（Canvas 零依赖：漂移粒子 + 连线 + 鼠标引力 + 网格纹理）；各内容页有专属动态渐变光晕。主按钮渐变 + 悬停光扫微交互；字体栈适配 苹方/鸿蒙/MiSans/雅黑；全站响应式，**手机 / 平板 / 桌面均适配**（后续所有改动保持移动端可用）。
...
assets/css/style.css  全局样式（Apple 风 / 视频封面 / 时间轴 / 响应式 / 打印）
assets/video/cover.mp4  封面与登录页背景视频（Mixkit 免费素材，4.1MB 自托管）
assets/js/
  ...
  bg-canvas.js  登录页科技风粒子网络背景（Canvas 零依赖）
```
